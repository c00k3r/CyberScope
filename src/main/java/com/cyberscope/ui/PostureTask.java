 ...
 *
 * <p>Shared by the Dashboard and the Vulnerabilities page. They ask the same
 * question -- "score every target's latest scan against today's index" -- and
 * differ only in what they draw from the answer: the Dashboard takes the roll-up
 * and the top five actions, the Vulnerabilities page takes the full ranked list.
 * Two copies of this loader would be two places to fix the next time the scoring
 * pipeline changes.
 *
 ...
 * working.
 */
final class PostureTask extends Task<NetworkPosture> {

    /**
     * How many targets the dashboard summarises.
     *
     * <p>A cap on TARGETS, not on findings. The Vulnerabilities page shows every
     * finding of every target this loads -- 15 services produced 211 findings in
     * testing -- so the cap bounds the query cost, not the answer.
     */
    static final int MAX_TARGETS = 40;

    private final AppContext context;

    PostureTask(AppContext context) {
