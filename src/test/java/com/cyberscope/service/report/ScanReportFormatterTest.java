...
class ScanReportFormatterTest {
    ...
    private static final Instant STARTED =
            Instant.parse("2026-08-19T15:42:11Z");

    private static final Service PROBED =
            new Service(
                    "http",
                    "SimpleHTTPServer",
                    "0.6",
                    "Python 3.11.15",
                    List.of("cpe:/a:python:simplehttpserver:0.6"),
                    DetectionMethod.PROBED,
                    10
            );

    private static final Service GUESSED =
            new Service(
                    "http-proxy",
                    "",
                    "",
    ...
    private static ValidatedTarget target(String value) {
        try {
            ...
            throw new AssertionError(
                    "Test target should be valid: " + value,
                    e
            );
        }
    }
    ...
    private static String format(
        ...
        return ScanReportFormatter.format(
                run(warnings),
                hosts,
                ZoneOffset.UTC
        );
    ...
    void headerIsComplete() {
        ...
        assertTrue(
                out.contains("Target      : 127.0.0.1"),
                out
        );

        assertTrue(
                out.contains("Scan type   : Quick"),
                out
        );

        assertTrue(
                out.contains("2026-08-19 15:42:11"),
                out
        );

        assertTrue(
                out.contains("Duration    : 6.5 s"),
                out
        );

        assertTrue(
                out.contains(
                        "nmap -sV -T4 -F -oX /tmp/x.xml 127.0.0.1"
                ),
                out
        );
    }
    ...
    void rendersProbedService() {
        ...
        assertTrue(
                out.contains("8080/tcp"),
                out
        );

        assertTrue(
                out.contains(
                        "SimpleHTTPServer 0.6 (Python 3.11.15)"
                ),
                out
        );

        assertTrue(
                out.contains("probed (conf 10)"),
                out
        );

        assertFalse(
                out.contains("Note: services marked 'table'"),
    ...
    void rendersTableGuessWithWarningFootnote() {
        String out =
                format(List.of(hostWith(GUESSED)), "");

        assertTrue(
                out.contains("http-proxy"),
                out
        );

        assertTrue(
                out.contains("table  (conf 3)"),
                out
        );
        ...
        assertFalse(
                out.contains("http-proxy  http-proxy"),
    ...
    void rendersNoHosts() {
        ...
        assertTrue(
                out.contains("No hosts were found"),
                out
        );

        assertTrue(
                out.contains(
                        "0 hosts scanned, 0 up, 0 open ports total"
                ),
                out
        );
    }
    ...
    void rendersHostWithNoOpenPorts() {
        ...
        assertTrue(
                out.contains("10.0.0.99  [DOWN]"),
                out
        );

        assertTrue(
                out.contains("No open ports found."),
                out
        );

        assertTrue(
                out.contains("1 host scanned, 0 up"),
                out
        );
    }
    ...
    void columnsAreAligned() {
        ...
        String out =
                format(List.of(hostWith(longName)), "");

        assertTrue(
                out.contains("A".repeat(60) + " 1.0"),
                "long product must not be truncated"
        );
        ...
        assertEquals(
                3,
                tableLines.size(),
                "header, rule and one data row"
        );

        assertTrue(
                tableLines.get(1).startsWith("   --------  -----"),
                "the rule row must match the column layout: "
                        + tableLines.get(1)
        );
    }
