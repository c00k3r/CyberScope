...
class NmapCommandBuilderTest {
    ...
    private static ValidatedTarget target(String value) {
        try {
            ...
            throw new AssertionError("Test target should be valid: " + value, e);
        }
    }
    ...
    void alwaysRequestsXml(ScanType type) throws Exception {
        ...
        assertEquals(
                "/tmp/scan.xml",
                cmd.get(i + 1),
                "-oX must be followed by the path"
        );
    }
    ...
    @ParameterizedTest
    @EnumSource(ScanType.class)
    @DisplayName("the timeout budget grows with the number of addresses")
    void timeoutScalesWithRange(ScanType type) {
        long single = type.timeoutFor(1).toSeconds();
        long full = type.timeoutFor(256).toSeconds();

        assertTrue(
                full > single,
                "a /24 must get more time than a single host"
        );

        assertEquals(
                single,
                type.timeoutFor(0).toSeconds(),
                "a count below 1 must not reduce the base budget"
        );
    }
    ...
    void pathWithSpacesStaysOneArgument() throws Exception {
        List<String> cmd =
                NmapCommandBuilder.build(
                        ScanType.QUICK,
                        target("127.0.0.1"),
                        Path.of("/tmp/my scans/out.xml")
                );

        assertEquals(
                "/tmp/my scans/out.xml",
                cmd.get(cmd.indexOf("-oX") + 1)
        );
    }

    @Test
    @DisplayName("invalid targets are rejected by TargetValidator")
    void rejectsInvalidTargets() {
        ...
        for (String invalid : invalidTargets) {
            assertThrows(
                    InvalidTargetException.class,
                    () -> TargetValidator.validate(invalid)
            );
        }
    }
    ...
    void describeRendersCommand() throws Exception {
        assertEquals(
                "nmap -sV -T4 -F -oX /tmp/scan.xml 127.0.0.1",
